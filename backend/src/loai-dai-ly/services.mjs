...
class LoaiDaiLyService {
  ...
  async getAllLoaiDaiLy() {
    const queryString = 'SELECT MaLoaiDaiLy as maloaidaily, TenLoaiDaiLy as tenloaidaily FROM inventory.LoaiDaiLy WHERE DeletedAt IS NULL';
    const result = await query(queryString);
    return result.rows;
  }

  async getLoaiDaiLy(maLoaiDaiLy) {
    const queryString = 'SELECT MaLoaiDaiLy as maloaidaily, TenLoaiDaiLy as tenloaidaily FROM inventory.LoaiDaiLy WHERE MaLoaiDaiLy = $1 AND DeletedAt IS NULL';
