...
class DonViTinhService {
  async createDonViTinh(tenDonViTinh) {
    ...
    // Return with consistent property naming
    return { madonvitinh: maDonViTinh };
  }

  async getAllDonViTinh() {
    const queryString = 'SELECT MaDonViTinh as madonvitinh, TenDonViTinh as tendonvitinh FROM inventory.DonViTinh WHERE DeletedAt IS NULL';
    const result = await query(queryString);
    return result.rows;
  }

  async getDonViTinh(maDonViTinh) {
    const queryString = 'SELECT MaDonViTinh as madonvitinh, TenDonViTinh as tendonvitinh FROM inventory.DONVITINH WHERE MaDonViTinh = $1 AND DeletedAt IS NULL';
